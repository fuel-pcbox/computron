...
#define DEFAULT_RM8_reg8( helper, name ) \
	void name() { \
		...
		modrm_update8( rm, helper( value, *treg8[rmreg( rm )], 8 )); \
	}
...
#define DEFAULT_RM16_reg16( helper, name ) \
	void name() { \
		...
		modrm_update16( rm, helper( value, *treg16[rmreg( rm )], 16 )); \
	}
...
#define DEFAULT_RM8_imm8( helper, name ) \
	void name() { \
		...
		modrm_update8( cpu_rmbyte, helper( value, cpu_pfq_getbyte(), 8 )); \
	}

#define DEFAULT_RM16_imm16( helper, name ) \
	void name() { \
		word value = modrm_read16( cpu_rmbyte ); \
		modrm_update16( cpu_rmbyte, helper( value, cpu_pfq_getword(), 16 )); \
	}

#define DEFAULT_RM16_imm8( helper, name ) \
	void name() { \
		word value = modrm_read16( cpu_rmbyte ); \
		modrm_update16( cpu_rmbyte, helper( value, signext( cpu_pfq_getbyte() ), 16 )); \
	}
